...
$effedUpLineCount = FALSE;
$file = fopen($target_dir.$theFile, "r");
if ($file) {
      while (!feof($file)) {
               $aline = fgets($file);
               //print strlen($aline)."<br>";
               if(strlen($aline) != 1160){//look for normal lines
                   if(strlen(trim($aline)) != 0){//look for zero line at the end of the file as CIMS is cool with that
                     $effedUpLineCount = TRUE;
                   }
               }
               ...
               $line[$i]['Distributor Customer Number'] = trim(substr($aline,5,20)); 
               ...
               $line[$i]['Transaction Date'] = trim(substr($aline,117,8));
               ...
               $line[$i]['Extra Charge 3'] = trim(substr($aline,686,14));
               $line[$i]['Extra Charge 4'] = trim(substr($aline,700,14)); 
               $line[$i]['Extra Charge 5'] = trim(substr($aline,714,14)); 
               ...
               $line[$i]['Supplier ID'] = trim(substr($aline,1138,20));
               $i++;
...
if(isset($_GET['invoice'])){$invoiceNumberToDisplay = $_GET['invoice'];}


foreach($line as $key=>$value){
    $z = 0;
        if(trim($value['Invoice Number']) == $invoiceNumberToDisplay || isset($_GET['all'])){
...
$fileDispaly = explode('/',$theFile);
if($invoiceNumberToDisplay != ''){
print '<h2>Ascii File Checker Breakdown V2 (iTrade)</h2>'.$fileDispaly[1].'<br/><br/>';
print 'Total Lines = '.($i - 1).' | Current Invoice = '.($invoiceNumberToDisplay).' | Total Invoices = '.($invoiceCount - 1).' | <a href="./ascii_checker_v3.php?invoice='.($invoiceNumberToDisplay).'&file='.$theFile.'" target="_BLANK">Trouble Shoot</a> | <a href="./ascii_checker_vLoop.php?file='.$theFile.'" target="_BLANK">Math Loop File</a> | <a href="./ascii_checker_v2.php?invoice='.($invoiceNumberToDisplay).'&cat=yes&file='.$theFile.'">Get Catagory Info</a> | <a href="./ascii_checker_v2.php?invoice='.($invoiceNumberToDisplay).'&all=yes&file='.$theFile.'">See All</a><br><br>';
foreach($invoiceNumbers as $key => $value){
    if(trim($value) != ''){
        $spanA = '<span>';
        if(in_array(trim($value), $nonAsciiDetection)){$spanA = '<span style="color: red;">';}
    print '<a href="./ascii_checker_v2.php?invoice='.trim($value).'&file='.$theFile.'">'.$spanA.trim($value).'</span></a>';
    if($key+2 < count($invoiceNumbers)){ print ' | ';}
    }
}
print '<br/><br/>';
}

if($effedUpLineCount){ print "<h3 style=\"color: red;\">Spec issue detected with file structure!</h3>";}
...
if($tableHeader != ''){
    ...
    print 'Header total'.$headertotalDispaly.' | Calculated Total = '.$calculatedInvoiceTotalForMatching.' | Variance tolerable = '.round(abs((($precentVarianceAllowed / 100) * $invoiceTotalForHeaderDetailMatch)),2).' | '.'Variance '.$sign.round($variance, 2).' <i>-> note: that Prod is set to only allow 75 cent variance, not sure why.</i>';
    //2.4% variance is ok
    $calGSTRate = 0;
    $calPSTRate = 0;
    ...
    if(abs($invoiceTotalForHeaderDetailMatch - $calculatedInvoiceTotalForMatching - $extraCharges) > abs((($precentVarianceAllowed / 100) * $invoiceTotalForHeaderDetailMatch))){print '<br><span style="color: red;">Header Detail Mismatch!!! '.round(($invoiceTotalForHeaderDetailMatch - ($invoiceTotalCalPrice + $invoiceTotalCalGST + $invoiceTotalCalPST)), 2).'</span>';}
    print '<br> Calculated Amount Before Tax/Fee\'s = '.($invoiceTotalCalPrice).' | Calculated GST = '.$invoiceTotalCalGST.' @ '.$calGSTRate.'% | Calculated PST = '.$invoiceTotalCalPST.' @ '.$calPSTRate.'% | Calculated Enviro Fee = '.$enviroFees;
    print '<br> Calculated Items with GST = '.$numberOfItemsWithGst.' | Calculated Total For Items With GST = '.$itemValueWithGst;
    print '<br> Calculated Items with PST = '.$numberOfItemsWithPst.' | Calculated Total For Items With PST = '.$itemValueWithPst;
    print '<table border = 1><tr>'.$tableHeader.'</tr>'.$tableRows.'</table>';
}

if($_GET['cat'] ==  'yes'){
//var_dump($nonAsciiDetection);
if(isset($theFile)){
print '<hr/>Cat Maker :)<br>';
foreach($catMaker as $key=>$val){
    if($key != ''){
    print $key.'|'.$val['Pack Size'].'|'.$val['Description'].'<br>';
    }
 }
}

//$invoiceToVendorCodeMaker
if(isset($theFile)){
print '<hr/>Customer Number Maker :)<br>';
print '<br>Invoice No|DCN<br>';
foreach($invoiceToVendorCodeMaker as $key=>$val){
    if($key != ''){
    print $key.'|'.$val['DCN'].'<br>';
    }
 }
}
}
